 ...
 */
function decodePinCode(bytes, minPinLength) {
    ...
    for (const byte of data) {
        ...
        const low = byte & 0x0f;
        if (high > 9 || low > 9) {
            logger_1.logger.debug(`PIN code ${data.length} bytes long is neither ASCII digits nor BCD, reporting it as hex`, NS);
    ...
    nimly_pro_lock_actions: {
        ...
        convert: (model, msg, publish, options, meta) => {
            ...
            if (msg.data["257"] !== undefined) {
                ...
                const minPinLength = typeof reportedMinPinLength === "number" && reportedMinPinLength > 0 ? reportedMinPinLength : defaultMinPinLength;
