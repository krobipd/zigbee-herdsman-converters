 ...
 */
const firmwareSupportFeaturesVersion = (device, targetVersion, model, type) => {
    if (device === undefined || targetVersion === undefined || model === undefined || type === undefined)
        return true;
    if (!device.softwareBuildID)
        return false;
    if (device.modelID !== model)
        return true;
    const currentParts = device.softwareBuildID.split(".").map((part) => Number(part));
...
const parseSonoffTpWgzbaTimePeriod = (value, key) => {
    if (!utils.isString(value)) {
...
const withConditionalExpose = (extend, predicate) => {
...
const withConditionalEnumValues = (extend, gatedValues, isSupported) => {
    const expose = (device, options) => {
        const items = (extend.exposes ?? []).flatMap((item) => (typeof item === "function" ? item(device, options) : [item]));
        if (isSupported(device))
            return items;
        return items.map((item) => {
            if (!(item instanceof exposes.Enum))
                return item;
...
const isBasicZB1GSPFirmwareAtLeast130 = (device) => utils.isDummyDevice(device) || firmwareSupportFeaturesVersion(device, "1.3.0", "BASIC-ZB1GSP", "higher");
...
const fzLocal = {
    ...
    snzb_09p_alarm_status: {
        ...
        convert: (model, msg, publish, options, meta) => {
            ...
            const alarmType = { 0: "none", 1: "manual", 2: "scene" }[status];
            return { alarm_type: alarmType, siren_on: alarmType === "none" ? "OFF" : "ON" };
        },
    },
...
const snzb02dr2SensorStateLookup = { unbound: 0x00, online: 0x01, offline: 0x02, restored: 0x03 };
const snzb02dr2StatusLookup = { unbound: 0x00, using: 0x01, offline: 0x02 };
const snzb02dr2SourceTemperatureKeys = ["source_1_temperature", "source_2_temperature"];
const snzb02dr2SourceHumidityKeys = ["source_1_humidity", "source_2_humidity"];
const snzb02dr2SourceTemperatureStateKeys = ["source_1_temperature_state", "source_2_temperature_state"];
const snzb02dr2SourceHumidityStateKeys = ["source_1_humidity_state", "source_2_humidity_state"];
const snzb02dr2SourceStateKeys = [...snzb02dr2SourceTemperatureStateKeys, ...snzb02dr2SourceHumidityStateKeys];
const firmwareAtLeast = (device, targetVersion) => {
    if (!device)
        return false;
...
const snzb02dr2GateExposesByFirmware = (extend, remoteSourceItems) => {
    const originalExposes = extend.exposes ?? [];
    const toZigbee = extend.toZigbee?.map((converter) => {
        const convertSet = converter.convertSet;
        const convertGet = converter.convertGet;
        const assertFirmware = (key, meta) => {
            if (firmwareAtLeast(meta.device, snzb02dr2RemoteSourceMinFirmware) === remoteSourceItems)
                return;
            if (remoteSourceItems) {
                throw new Error(`SNZB-02DR2 ${key} requires firmware ${snzb02dr2RemoteSourceMinFirmware} or later`);
            }
            throw new Error(`SNZB-02DR2 ${key} uses the legacy remote source protocol; use source_1/source_2 properties on firmware ${snzb02dr2RemoteSourceMinFirmware} or later`);
        };
        return {
            ...converter,
            convertSet: convertSet === undefined
                ? undefined
                : async (entity, key, value, meta) => {
                    assertFirmware(key, meta);
                    return await convertSet(entity, key, value, meta);
                },
            convertGet: convertGet === undefined
                ? undefined
    ...
    return {
        ...
        exposes: [
            (device, options) => {
                if (firmwareAtLeast(device, snzb02dr2RemoteSourceMinFirmware) !== remoteSourceItems)
                    return [];
                const result = [];
...
const parseSonoffSnzb02dr2RemoteSourceElements = (elements) => {
    ...
    for (let index = 0; index < sensorCount; index++) {
        ...
        if ((type !== 0x00 && type !== 0x01) || (id !== 0x00 && id !== 0x01) || ![0x00, 0x01, 0x02, 0x03].includes(state))
            return;
        if ((valueLength !== 0 && valueLength !== 2) || offset + 4 + valueLength > elements.length)
            return;
        const key = `${type}:${id}`;
        if (seen.has(key))
            return;
        ...
        if (type === 0x01 && value !== undefined && ((value[0] ?? 0) | ((value[1] ?? 0) << 8)) > 10000)
            return;
        offset += valueLength;
        items.push({ type, id, state, value });
    }
    return offset === elements.length ? items : undefined;
};
const sonoffExtend = {
    zbminiR2ExternalSwitchActions: () => {
        const clusterName = "customClusterEwelink";
        const actionLookup = {
            2: "double_click",
            3: "long_press",
        };
        const supportNewActions = (device) => device.modelID === "ZBMINIR2" && firmwareAtLeast(device, "1.1.0");
    ...
    snzb02dr2RemoteSource: () => {
        ...
        const remoteSourceExposes = [
            ...
            e
                ...
                .withValueStep(0.1)
                .withUnit("°C")
                .withDescription("Displays the bound remote sensor's temperature on the device screen, updating automatically every 30 minutes or manually by briefly pressing the button on the back of the SNZB-02DR2."),
            e
                .enum("source_1_temperature_state", ea.ALL, Object.keys(snzb02dr2SensorStateLookup))
                .withDescription("State of the source 1 temperature item. State-only online/restored updates do not overwrite the stored measurement."),
            e
                ...
                .withValueStep(0.1)
                .withUnit("%")
                .withDescription("Displays the bound remote sensor's humidity on the device screen, updating automatically every 30 minutes or manually by briefly pressing the button on the back of the SNZB-02DR2."),
            e
            ...
            e
                ...
                .withDescription("State of the source 2 temperature item. State-only online/restored updates do not overwrite the stored measurement."),
            e
        ...
        const encodeRemoteSourceValue = (type, value, key) => {
            ...
            if (type === 0x00 && (raw < snzb02dr2RemoteSourceTemperatureRange.min * 100 || raw > snzb02dr2RemoteSourceTemperatureRange.max * 100)) {
                throw new Error(`Invalid ${key}: temperature must be between ${snzb02dr2RemoteSourceTemperatureRange.min} and ${snzb02dr2RemoteSourceTemperatureRange.max}`);
            }
            if (type === 0x01 && (raw < snzb02dr2RemoteSourceHumidityRange.min * 100 || raw > snzb02dr2RemoteSourceHumidityRange.max * 100)) {
                throw new Error(`Invalid ${key}: humidity must be between ${snzb02dr2RemoteSourceHumidityRange.min} and ${snzb02dr2RemoteSourceHumidityRange.max}`);
            }
            return [raw & 0xff, (raw >> 8) & 0xff];
        };
        const readRemoteSource = async (entity) => {
            await entity.read(clusterName, [0x600e, 0x601e]);
        };
        const writeRemoteSourceStatus = async (entity, status) => {
            await entity.write(clusterName, { [0x600e]: { value: status, type: zigbee_herdsman_1.Zcl.DataType.UINT8 } }, undefined);
        };
        const writeRemoteSourceItems = async (entity, items) => {
        ...
        const buildCachedRemoteSourceSnapshot = (meta, overrides) => {
            ...
            for (const definition of snzb02dr2RemoteSourceItemDefinitions) {
                if (overrideKeys.has(`${definition.type}:${definition.id}`))
                    continue;
                const cachedValue = meta.state?.[definition.valueKey];
                const cachedState = meta.state?.[definition.stateKey];
                const state = typeof cachedState === "string"
                    ? snzb02dr2SensorStateLookup[cachedState]
                    : typeof cachedValue === "number"
                        ? snzb02dr2SensorStateLookup.online
                        : undefined;
                if (state === undefined || state === snzb02dr2SensorStateLookup.unbound)
                    continue;
                const value = typeof cachedValue === "number" && (state === snzb02dr2SensorStateLookup.online || state === snzb02dr2SensorStateLookup.restored)
        ...
        const writeRemoteSource = async (entity, meta, items) => {
            ...
            try {
                ...
                if (error instanceof Error && error.message.includes("UNSUPPORTED_ATTRIBUTE")) {
                    throw new Error("The device rejected remote source attribute 0x601E: firmware 1.0.5 or later is required. Update the device via OTA and retry.");
                }
                throw error;
            }
            return gateWasEnabled;
        ...
        const fromZigbee = [
            {
                ...
                convert: (model, msg, publish, options, meta) => {
                    ...
                    if (statusRaw !== undefined && Object.values(snzb02dr2StatusLookup).includes(statusRaw)) {
                        ...
                        if (items?.length === 0) {
                            ...
                            for (const item of items) {
                                ...
                                if (item.state === snzb02dr2SensorStateLookup.unbound) {
                                    ...
                                    const raw = item.type === 0x00 ? snzb02dr2ReadInt16LE(item.value, 0) : (item.value[0] ?? 0) | ((item.value[1] ?? 0) << 8);
        ...
        const toZigbee = [
            {
                ...
                convertSet: async (entity, key, value, meta) => {
                    if (!firmwareAtLeast(meta.device, snzb02dr2RemoteSourceMinFirmware)) {
                        throw new Error(`SNZB-02DR2 ${key} requires firmware ${snzb02dr2RemoteSourceMinFirmware} or later; use external_temperature/external_humidity on older firmware`);
                    }
                    const returnState = {};
                    if (key === "remote_source_status") {
                        utils.assertString(value, key);
                        const status = snzb02dr2StatusLookup[value];
                        if (status === undefined) {
                    ...
                    if (snzb02dr2SourceTemperatureKeys.includes(key) || snzb02dr2SourceHumidityKeys.includes(key)) {
                        ...
                        utils.assertString(value, key);
                        const state = snzb02dr2SensorStateLookup[value];
                        if (state === undefined) {
    ...
    remoteSensorData: () => {
        ...
        const STATE_CODE = { enable: 0x01, disable: 0x00 };
        const buildItem = (sensorType, sensorState, valueLen, writeFn) => {
            const item = Buffer.alloc(4 + valueLen);
            item[0] = sensorType;
            item[1] = 0x00; // SensorId (default 0)
            item[2] = sensorState;
            item[3] = valueLen;
            if (valueLen > 0 && writeFn)
                writeFn(item, 4);
            return item;
        };
        const writePayload = async (entity, items, meta) => {
            ...
            await entity.write(clusterName, { [ATTR_ID]: { value: { elementType: zigbee_herdsman_1.Zcl.DataType.UINT8, elements: payload }, type: zigbee_herdsman_1.Zcl.DataType.ARRAY } }, utils.getOptions(meta.mapped, entity));
        };
        const isOnline = (meta) => meta.state.remote_sensors_state !== "disable";
        const exposes = [
            e
                .enum("remote_sensors_state", ea.STATE_SET, ["enable", "disable"])
                .withDescription("The remote temperature‑humidity source shares the same display area with the date. When the remote temperature‑humidity source is enabled, the date will no longer be shown. Note: wake up the device by pressing the button on the back before changing this value.")
                .withCategory("config"),
            e
                ...
                .withUnit("°C")
                .withValueStep(0.1)
                .withDescription("Remote temperature value displayed on the E-ink screen. Note: wake up the device by pressing the button on the back before changing this value.")
                .withCategory("config"),
            e
                ...
                .withValueMax(95)
                .withUnit("%")
                .withDescription("Remote humidity value displayed on the E-ink screen. Note: wake up the device by pressing the button on the back before changing this value.")
                .withCategory("config"),
            e
                ...
                .withUnit("hPa")
                .withValueStep(0.1)
                .withDescription("Remote atmospheric pressure value displayed on the E-ink screen. Note: wake up the device by pressing the button on the back before changing this value.")
                .withCategory("config"),
        ];
        const toZigbee = [
            {
            ...
            },
            {
                ...
                convertSet: async (entity, key, value, meta) => {
                    ...
                    const stateCode = STATE_CODE[value] ?? 0x01;
                    await writePayload(entity, [0x00, 0x01, 0x02].map((type) => buildItem(type, stateCode, 0)), meta);
                    return { state: { remote_sensors_state: value } };
                },
            },
        ];
    ...
    getCurrentWeatherInfo02UL: () => {
        ...
        const expose = e
            ...
            .withCategory("config")
            .withDescription("Weather shown on the E-ink screen. Selecting a value sends the default coordinates to the device and is applied on its next weather request. Note: wake up the device by pressing the button on the back before changing this value.");
        const toZigbee = [
            {
                key: [weatherKey],
                convertSet: async (entity, key, value, meta) => {
                    await entity.write(clusterName, { longitude: DEFAULT_LONGITUDE_MICRO, latitude: DEFAULT_LATITUDE_MICRO }, utils.getOptions(meta.mapped, entity));
        ...
        const fromZigbee = [
            {
                ...
                convert: async (model, msg, publish, options, meta) => {
                    if (!(msg.data instanceof Buffer))
                        return;
                    const parsedRawCommand = (0, sonoff_1.parseSWVZFRawZclCommand)(msg.data);
                    if (!parsedRawCommand || parsedRawCommand.commandId !== commandId)
                        return;
                    const payload = parsedRawCommand.payload;
                    if (payload.length !== 9) {
                        logger_1.logger.warning(`getCurrentWeatherInfo02UL: invalid payload length=${payload.length}`, NS);
                        return;
                    }
                    const type = payload.readUInt8(0);
                    if (type !== 0x00) {
                        logger_1.logger.warning(`getCurrentWeatherInfo02UL: unsupported device type=${type}`, NS);
                        return;
                    }
                    ...
                    if (weatherValue === undefined) {
                        logger_1.logger.warning(`getCurrentWeatherInfo02UL: no weather selected (weather=${JSON.stringify(selectedWeather)}), falling back to sunny`, NS);
                    }
                    const requestTransactionSequenceNumber = msg.meta.zclTransactionSequenceNumber;
                    const requestDirection = msg.meta.frameControl?.direction ?? zigbee_herdsman_1.Zcl.Direction.SERVER_TO_CLIENT;
                    const responseDirection = requestDirection === zigbee_herdsman_1.Zcl.Direction.CLIENT_TO_SERVER ? zigbee_herdsman_1.Zcl.Direction.SERVER_TO_CLIENT : zigbee_herdsman_1.Zcl.Direction.CLIENT_TO_SERVER;
                    await msg.endpoint.commandResponse(clusterName, replyCommandName, { data: [type, 0x01, weatherValue ?? 0x00] }, {
                        disableDefaultResponse: true,
                        direction: responseDirection,
                        manufacturerCode: zigbee_herdsman_1.Zcl.ManufacturerCode.SHENZHEN_COOLKIT_TECHNOLOGY_CO_LTD,
                    }, requestTransactionSequenceNumber);
                    logger_1.logger.info(`getCurrentWeatherInfo02UL: response sent type=${type} weather=${JSON.stringify(selectedWeather)} weatherValue=${weatherValue ?? 0x00}`, NS);
                },
            },
        ];
        ...
        const toZigbee = [
            {
                ...
                convertSet: async (entity, key, value, meta) => {
                    ...
                    await entity.write(clusterName, {
                        temperatureControlThreshold: buildSonoffStruct(zigbee_herdsman_1.Zcl.DataType.INT16, [Math.round(low * 100), Math.round(high * 100)]),
                    });
    ...
    manualDefaultSettings: (hasFlowMeter) => {
        ...
        const publishManualSettings = (settings) => ({
        ...
        const toZigbee = [
            {
                ...
                convertSet: async (entity, key, value, meta) => {
                    ...
                    if (hasFlowMeter && (typeof nextValue.irrigation_mode !== "string" || modeMap[nextValue.irrigation_mode] === undefined)) {
                        logger_1.logger.error("irrigation_mode invalid value, expected one of: duration, capacity.", NS);
                        return;
                    }
                    ...
                    if (hasFlowMeter) {
                        ...
                        if (parsedCapacityUnit === undefined) {
                            logger_1.logger.error("irrigation_amount_unit invalid value, expected one of: us_gallon, liter.", NS);
                            return;
                        }
                    ...
                    const parseRequiredInt = (fieldName) => {
                        ...
                        if (!Number.isInteger(parsed)) {
                            logger_1.logger.error(`manual irrigation setting ${fieldName} expected integer.`, NS);
                            return;
                        }
                        return parsed;
                    };
                ...
                convert: (model, msg, publish, options, meta) => {
                    ...
                    if (typeof amount === "number") {
                        ...
                        if (sourceUnit && sourceUnit !== waterFlowUnit) {
                            const irrigationAmountLiters = typeof amountRealLiters === "number" ? amountRealLiters : amount * SWVZNELitersPerWaterFlowUnit[sourceUnit];
    ...
    readConsumptionRecord(clusterName, commandName, model, targetVersion) {
        const expose = (device) => utils.isDummyDevice(device) || firmwareSupportFeaturesVersion(device, targetVersion, model, "lower")
            ? [
    ...
    readRecordWithMultiConsumption: (args) => {
        const clusterName = "customClusterEwelink";
        const commandName = "readElectricityRecords";
        const recordTypes = {
            hour: 0x00,
            day: 0x01,
            month: 0x02,
        ...
        const getRecordsTimeRangeLog = (records) => {
        ...
        const attachReadRecordTimeRange = (records, subCommand, cache, lastPackageTime) => {
            if (records.length === 0) {
                return records;
            }
            ...
            return records.map((record, index) => {
                ...
                if (subCommand === recordTypes.year) {
                    const lastPackageUtcSeconds = (0, sonoff_1.deviceLocal2000ToUTCSeconds)(timeAnchor, cache.offsetSeconds);
                    start =
                        getMonthStartUtcSeconds(lastPackageUtcSeconds, -(records.length - 1 - index), cache.offsetSeconds) -
                            sonoff_1.YEAR_2000_IN_UTC +
                            cache.offsetSeconds;
                    end =
        ...
        const startReadRecordCacheWithTimeout = (cacheKey, cache, publish) => {
            clearReadRecordCacheTimeout(cache);
            cache.timeout = setTimeout(() => {
                ...
                readRecordCaches.delete(cacheKey);
                const timeoutRecords = cache.subCommand === recordTypes.year
        ...
        const sendReadRecordPacket = async (entity, cache, packetIndex) => {
            const data = [cache.subCommand];
            ...
            if (electricityRecordSubCommands.includes(cache.subCommand)) {
                ...
                data.push(cache.electricalFlag, ...(0, sonoff_1.toUInt32LEBytes)(cache.startTime), ...(0, sonoff_1.toUInt32LEBytes)(cache.endTime), ...(0, sonoff_1.toUInt16LEBytes)(packetIndex));
            }
            else if (cache.subCommand === energyRecord24hSubCommand) {
                data.push(...(0, sonoff_1.toUInt16LEBytes)(packetIndex));
            }
            else {
                throw new Error(`Cannot read ${cache.type} packet ${packetIndex}, unsupported sub command ${cache.subCommand}`);
            }
            await entity.command(clusterName, commandName, { data }, { disableDefaultResponse: true, disableResponse: false });
        };
        // The device encodes local wall-clock time as seconds since 2000-01-01.
        const parseRecordTime = (value, field, offsetSeconds) => {
            if (!utils.isString(value)) {
                throw new Error(`Invalid read_electricity_records.${field}, expected ISO 8601 datetime with timezone`);
            }
            const utcSeconds = (0, sonoff_1.parseIsoWithOffsetToUtcSeconds)(value);
            if (utcSeconds === undefined || utcSeconds < sonoff_1.YEAR_2000_IN_UTC) {
                throw new Error(`Invalid read_electricity_records.${field}, expected ISO 8601 datetime with timezone at or after 2000-01-01T00:00:00Z`);
            }
            return (0, sonoff_1.utcToDeviceLocal2000Seconds)(utcSeconds, offsetSeconds);
        };
        const getRecordTimeOffsetSeconds = (value, field) => {
            if (!utils.isString(value)) {
                throw new Error(`Invalid read_electricity_records.${field}, expected ISO 8601 datetime with timezone`);
            }
            if (value.endsWith("Z")) {
        ...
        const formatRecordTime = (deviceSeconds, cache) => {
            return (0, sonoff_1.formatUtcSecondsToIsoWithOffset)((0, sonoff_1.deviceLocal2000ToUTCSeconds)(deviceSeconds, cache.offsetSeconds), cache.offsetSeconds);
        };
        const readElectricityRecordsExpose = e
            ...
            .withFeature(e.binary("with_reverse_energy", ea.SET, true, false).withDescription("Request reverse energy. Used by hour/day/month/year reads."));
        if (args.withCost) {
            readElectricityRecordsExpose.withFeature(e.binary("with_cost", ea.SET, true, false).withDescription("Request electricity cost. Used by hour/day/month/year reads."));
        }
        readElectricityRecordsExpose
            ...
            .withFeature(e.text("end_time", ea.SET).withDescription("Record end time in ISO 8601 format with timezone. Example: 2026-05-21T12:00:00+08:00."));
        ...
        readAllElectricityRecordsExpose.withFeature(e.binary("with_timestamp", ea.SET, true, false).withDescription("Request the last timestamp in the response package."));
        const exposes = [
            readElectricityRecordsExpose,
        ...
        const expose = (device) => utils.isDummyDevice(device) || firmwareSupportFeaturesVersion(device, args.target, args.model, "higher") ? exposes : [];
        const fromZigbee = [
            {
                cluster: clusterName,
                type: ["raw"],
                convert: (model, msg, publish) => {
                    ...
                    if (bytes.length === 0) {
                        logger_1.logger.error("readRecordResp payload is empty", NS);
                        return;
                    }
                    logger_1.logger.info(`Received readRecordResp with payload: ${bytes.map((b) => b.toString(16).padStart(2, "0")).join(" ")}`, NS);
                    if (bytes.length < 2) {
                        logger_1.logger.error(`readRecordResp payload too short, expected at least 2 bytes but got ${bytes.length}`, NS);
                        return;
                    }
                    ...
                    if (type === undefined) {
                        logger_1.logger.error(`readRecordResp unknown sub command: ${subCommand}`, NS);
                        return;
                    }
                    ...
                    const cache = readRecordCaches.get(cacheKey);
                    if (cache === undefined) {
                        logger_1.logger.warning(`Ignoring readRecordResp ${type} because no active read is waiting for it`, NS);
                        return;
                    }
                    if (status !== 0) {
                        logger_1.logger.error(`readRecordResp failed with status=${status}, type=${type}`, NS);
                    ...
                    let lastPackageTime;
                    const records = [];
                    let packetElectricalFlag;
                    if (electricityRecordSubCommands.includes(subCommand)) {
                        if (responseData.length < 5) {
                            logger_1.logger.error(`readRecordResp ${type} payload too short, expected at least 5 bytes but got ${responseData.length}`, NS);
                            return;
                        }
                        ...
                        const recordByteLength = (withEnergy ? valueByteLength : 0) + (withReverseEnergy ? valueByteLength : 0) + (withCost ? valueByteLength : 0);
                        totalPacket = (0, sonoff_1.readUInt16LE)(responseData, 1);
                        currentPacket = (0, sonoff_1.readUInt16LE)(responseData, 3);
                        if (recordByteLength === 0) {
                            logger_1.logger.error(`readRecordResp ${type} has no requested value fields`, NS);
                        }
                        else {
                            ...
                            if (withTimestamp && subCommand !== recordTypes.year && dataEndIndex >= 9) {
                                dataEndIndex -= 4;
                                const packetLastPackageTime = (0, sonoff_1.readUInt32LE)(responseData, dataEndIndex);
                            ...
                            while (index + recordByteLength <= dataEndIndex) {
                                const record = {};
                                if (withEnergy) {
                                    record.energy = (0, sonoff_1.readUInt32LE)(responseData, index);
                                    index += valueByteLength;
                                }
                                if (withReverseEnergy) {
                                    record.reverse_energy = (0, sonoff_1.readUInt32LE)(responseData, index);
                                    index += valueByteLength;
                                }
                                if (withCost) {
                                    record.cost = (0, sonoff_1.readUInt32LE)(responseData, index) / 100;
                                    index += valueByteLength;
                                }
                                records.push(record);
                            }
                            if (index < dataEndIndex) {
                                logger_1.logger.warning(`readRecordResp ${type} packet=${currentPacket} has ${dataEndIndex - index} leftover bytes, ` +
                                    `expected ${recordByteLength} bytes per record`, NS);
                            }
                        }
                    }
                    else {
                    ...
                    if (currentPacket === undefined || totalPacket === undefined) {
                        logger_1.logger.error(`readRecordResp ${type} is missing packet indexes`, NS);
                        return;
                    }
                    if (totalPacket === 0 || currentPacket >= totalPacket) {
                        logger_1.logger.warning(`Ignoring readRecordResp ${type} packet=${currentPacket}, total packets=${totalPacket}`, NS);
                        return;
                    }
                    if (currentPacket !== cache.requestedPacket) {
                        logger_1.logger.warning(`Ignoring readRecordResp ${type} packet=${currentPacket}, expected packet=${cache.requestedPacket}`, NS);
                        return;
                    }
                    if (packetElectricalFlag !== undefined && cache.electricalFlag !== undefined && packetElectricalFlag !== cache.electricalFlag) {
                        logger_1.logger.warning(`Ignoring readRecordResp ${type} packet=${currentPacket} with unexpected electrical flag ${packetElectricalFlag}`, NS);
                        return;
                    }
                    if (lastPackageTime !== undefined &&
                        ...
                        logger_1.logger.warning(`readRecordResp unexpected last timestamp ${cache.type} packet=${currentPacket} has ` +
                            `${formatRecordTime(lastPackageTime, cache)}, expected between ` +
                            `${formatRecordTime(cache.startTime, cache)} and ` +
                            `${formatRecordTime(cache.endTime, cache)}`, NS);
                    }
                    clearReadRecordCacheTimeout(cache);
                    ...
                    if (cache.isPagedAllRecordRead) {
                        clearReadRecordCache(cacheKey);
                        logger_1.logger.info(`readRecordResp ${type} paged packet ${currentPacket}/${totalPacket - 1}${getRecordsTimeRangeLog(packetRecords)}, ` +
                            `publishing ${packetRecords.length} records`, NS);
                        publish({
                            all_electricity_records: JSON.stringify({
                                type,
                                page: currentPacket,
                    ...
                    if (currentPacket + 1 < totalPacket) {
                        ...
                        logger_1.logger.info(`readRecordResp ${type} packet ${currentPacket}/${totalPacket - 1}${getRecordsTimeRangeLog(packetRecords)}, ` +
                            `requesting packet ${nextPacket}`, NS);
                        setTimeout(() => {
                            if (readRecordCaches.get(cacheKey) !== cache) {
                                return;
                            }
                            sendReadRecordPacket(msg.endpoint, cache, nextPacket)
                                ...
                                .catch((error) => {
                                clearReadRecordCache(cacheKey);
                                logger_1.logger.error(`readRecordResp failed to request next packet for ${type}: ${String(error)}`, NS);
                    ...
                    clearReadRecordCache(cacheKey);
                    const collectedRecords = subCommand === recordTypes.year
                    ...
                    logger_1.logger.info(`readRecordResp ${type} final packet ${currentPacket}/${totalPacket - 1}${getRecordsTimeRangeLog(publishedRecords)}, ` +
                        `publishing ${publishedRecords.length} records`, NS);
                    const publishedPayload = {
                        type,
                        records: publishedRecords,
                    };
                    if (cache.startTime !== undefined)
                        publishedPayload.start = formatRecordTime(cache.startTime, cache);
                    if (cache.endTime !== undefined)
                        publishedPayload.end = formatRecordTime(cache.endTime, cache);
        ...
        const toZigbee = [
            {
                ...
                convertSet: async (entity, key, value, meta) => {
                    utils.assertObject(value, key);
                    const payload = value;
                    const subCommand = recordTypes[payload.type];
                    ...
                    let offsetSeconds = (0, sonoff_1.getRuntimeLocalOffsetSeconds)(Math.floor(Date.now() / 1000));
                    const cache = {
                        type: payload.type,
                        subCommand,
                        recordsByPacket: new Map(),
                        offsetSeconds,
                    };
                    if (electricityRecordSubCommands.includes(subCommand)) {
                        ...
                        if (!utils.isBoolean(withEnergy))
                            throw new Error(`Invalid ${key}.with_energy, expected boolean`);
                        if (!utils.isBoolean(withReverseEnergy))
                            throw new Error(`Invalid ${key}.with_reverse_energy, expected boolean`);
                        if (args.withCost && !utils.isBoolean(withCost))
                            throw new Error(`Invalid ${key}.with_cost, expected boolean`);
                        if (!utils.isBoolean(withTimestamp))
                            throw new Error(`Invalid ${key}.with_timestamp, expected boolean`);
                        ...
                        const electricalFlag = (withEnergy ? 0b0001 : 0) | (withReverseEnergy ? 0b0010 : 0) | (withCost ? 0b0100 : 0) | (withTimestamp ? 0b1000 : 0);
...
const trvzbTimerModeTempFzConvert = (model, msg, publish, options, meta) => {
...
exports.definitions = [
        ...
        extend: [
            m.deviceAddCustomCluster("customSonoffSnzb02dr2", {
                ...
                attributes: {
                    ...
                    remoteSourceItems: { name: "remoteSourceItems", ID: 0x601e, type: zigbee_herdsman_1.Zcl.DataType.ARRAY, write: true },
                },
                commands: {},
                commandsResponse: {},
            }),
            ...
            m.humidity(),
            m.bindCluster({ cluster: "genPollCtrl", clusterType: "input" }),
            snzb02dr2GateExposesByFirmware(m.enumLookup({
                ...
                fzConvert: (model, msg, publish, options, meta) => {
                    if (firmwareAtLeast(meta.device, snzb02dr2RemoteSourceMinFirmware))
                        return;
                    if (msg.data.temperatureSensorSelect === 0)
                        return { temperature_sensor_select: "internal" };
                    if (msg.data.temperatureSensorSelect === 1)
                        return { temperature_sensor_select: "external" };
                },
            }), false),
            snzb02dr2GateExposesByFirmware(m.numeric({
    ...
    },
    {
        ...
        configure: async (device, coordinatorEndpoint) => {
            ...
            await reporting.onOff(endpoint, { min: 1, max: 1800, change: 0 });
            await endpoint.read("customClusterEwelink", ["externalTriggerMode"], defaultResponseOptions);
        },
    },
    {
    ...
    },
    {
        ...
        extend: [
            ...
            withConditionalExpose(m.numeric({
        ...
        extend: [
            m.deviceAddCustomCluster("customClusterEwelink", {
                ...
                attributes: {
                    ...
                    alarmStatus: { name: "alarmStatus", ID: 0x202e, type: zigbee_herdsman_1.Zcl.DataType.UINT8 },
            ...
            // this entry after it.
            {
                onEvent: [
                    async (event) => {
                        ...
                        try {
                            await endpoint.read("customClusterEwelink", ["alarmStatus"], manufacturerOptions);
                        }
                        catch (error) {
                            ...
                            logger_1.logger.warning(`Failed to read alarmStatus of '${event.data.device.ieeeAddr}' on '${event.type}' (${error})`, NS);
                        }
                    },
                ],
                isModernExtend: true,
            ...
            withConditionalEnumValues(m.enumLookup({
            ...
            }), Object.keys(snzb09pAlarmSoundTypeChimeLookup), (device) => utils.isDummyDevice(device) || firmwareSupportFeaturesVersion(device, "1.1.9", "SNZB-09P", "higher")),
            m.enumLookup({
        ...
        extend: [
            m.deviceAddCustomCluster("customClusterEwelink", {
                ...
                attributes: {
                    ...
                    temperatureCalibration: { name: "temperatureCalibration", ID: 0x2003, type: zigbee_herdsman_1.Zcl.DataType.INT16, write: true },
                    humidityCalibration: { name: "humidityCalibration", ID: 0x2004, type: zigbee_herdsman_1.Zcl.DataType.INT16, write: true },
                    remoteSensorData: { name: "remoteSensorData", ID: 0x601e, type: zigbee_herdsman_1.Zcl.DataType.ARRAY, write: true },
                    longitude: { name: "longitude", ID: 0x5016, type: zigbee_herdsman_1.Zcl.DataType.INT32, write: true, min: -2147483648 },
                    latitude: { name: "latitude", ID: 0x5017, type: zigbee_herdsman_1.Zcl.DataType.INT32, write: true, min: -2147483648 },
                },
                commands: {
            ...
            m.numeric({
                ...
                attribute: "comfortTemperatureMin",
                entityCategory: "config",
                description: "Minimum temperature that is considered comfortable. The device will display a snowflake icon❄ when the temperature is lower than this value. Note: wake up the device by pressing the button on the back before changing this value.",
                valueMin: 0,
                valueMax: 50,
                scale: 100,
                valueStep: 0.1,
            ...
            m.numeric({
                ...
                attribute: "comfortTemperatureMax",
                entityCategory: "config",
                description: "Maximum temperature that is considered comfortable. The device will display a flame icon🔥 when the temperature is higher than this value. Note: wake up the device by pressing the button on the back before changing this value.",
                valueMin: 0,
                valueMax: 50,
                scale: 100,
                valueStep: 0.1,
            ...
            m.numeric({
                ...
                attribute: "comfortHumidityMin",
                entityCategory: "config",
                description: "Minimum humidity that is considered comfortable. The device will display an empty droplet icon💧 when the humidity is lower than this value. Note: wake up the device by pressing the button on the back before changing this value.",
                valueMin: 5,
                valueMax: 95,
                scale: 100,
                valueStep: 0.1,
            ...
            m.numeric({
                ...
                attribute: "comfortHumidityMax",
                entityCategory: "config",
                description: "Maximum humidity that is considered comfortable. The device will display a half‑filled droplet icon💧 when the humidity is higher than this value. Note: wake up the device by pressing the button on the back before changing this value.",
                valueMin: 5,
                valueMax: 95,
                scale: 100,
                valueStep: 0.1,
