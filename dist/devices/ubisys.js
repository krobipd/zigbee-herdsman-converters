...
function ubisysLd6DecodeOutputChannel(bytes) {
    if (bytes.length < 6)
        return undefined;
...
function ubisysLd6EncodeOutputChannel(channel) {
    ...
    if (fn < 0)
        throw new Error(`ubisys: unknown LD6 output function '${channel.function}'`);
    ...
    const scale = (v) => (v == null ? 0xffff : v <= 1 ? Math.round(v * 65536) : Number(v));
...
function ubisysLd6LightEndpoints(device) {
    if (utils.isDummyDevice(device))
        return [];
    return device.endpoints
        .filter((ep) => ep.inputClusters.includes(zigbee_herdsman_1.Zcl.Clusters.genOnOff.ID) && ep.inputClusters.includes(zigbee_herdsman_1.Zcl.Clusters.genLevelCtrl.ID))
        .map((ep) => ep.ID)
        .sort((a, b) => a - b);
}
...
const UBISYS_LD6_COLOR_TEMP_FALLBACK = [153, 500];
function ubisysLd6ColorCapabilities(device, endpointId) {
    if (utils.isDummyDevice(device))
        return 0;
    const endpoint = device.endpoints.find((ep) => ep.ID === endpointId);
    if (!endpoint?.inputClusters.includes(zigbee_herdsman_1.Zcl.Clusters.lightingColorCtrl.ID))
        return 0;
...
function ubisysLd6ColorTempRange(device, endpointId) {
            ...
            convert: (model, msg, publish, options, meta) => {
                ...
                if (msg.data.outputConfigurations != null) {
                    result.output_configuration = {
                        channels: msg.data.outputConfigurations
                ...
                if (msg.data.outputEndpoints != null) {
                    ...
                    const bitmap = msg.data.outputEndpoints;
    ...
    tz: {
        ...
        configure_device_setup: {
            ...
            convertGet: async (entity, key, meta) => {
                ...
                const customCluster = meta.device.customClusters["manuSpecificUbisysDeviceSetup"];
                (0, node_assert_1.default)(customCluster);
                await devMgmtEp.read("manuSpecificUbisysDeviceSetup", [customCluster.attributes.inputConfigurations.ID, customCluster.attributes.inputActions.ID], manufacturerOptions.ubisysNull);
            },
        },
        output_configuration: {
            ...
            convertSet: async (entity, key, value, meta) => {
                ...
                (0, node_assert_1.default)(attribute);
                ...
                logger_1.logger.info(`ubisys: LD6 output configuration written, re-interview '${meta.options.friendly_name}' to pick up the new endpoints`, NS);
            },
            convertGet: async (entity, key, meta) => {
                const devMgmtEp = meta.device.getEndpoint(232);
                const customCluster = meta.device.customClusters["manuSpecificUbisysDeviceSetup"];
                (0, node_assert_1.default)(customCluster);
                await devMgmtEp.read("manuSpecificUbisysDeviceSetup", [customCluster.attributes.outputConfigurations.ID, customCluster.attributes.outputEndpoints.ID], manufacturerOptions.ubisysNull);
            },
        },
    },
};
exports.definitions = [
    ...
    },
    {
        ...
        extend: [
            ubisys_1.ubisysModernExtend.addCustomClusterManuSpecificUbisysDeviceSetup(),
            ubisys_1.ubisysModernExtend.pollCurrentSummDelivered(5),
    ...
    },
    {
        ...
        exposes: (device, options) => {
            ...
            const channels = lights.length === 0 ? [{}] : lights.map((id, index) => ({ id, names: lights.length > 1 ? [`l${index + 1}`] : undefined }));
            for (const { id, names } of channels) {
                const capabilities = id === undefined ? UBISYS_LD6_CAPABILITY_XY | UBISYS_LD6_CAPABILITY_COLOR_TEMP : ubisysLd6ColorCapabilities(device, id);
                ...
                if (colorTemp)
                    light.withColorTemp(range).withColorTempStartup(range);
                if (xy)
                    light.withColor(["xy"]);
                result.push(...utils.exposeEndpoints(light, names));
                const effect = e.effect();
                if (xy)
                    effect.values.push("colorloop", "stop_colorloop");
