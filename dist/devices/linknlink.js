...
const AIR_KEYS = ["freq", "trith", "absence_timeout", "radar_enable", "lx_interval", "sht_interval", "lx_thread1", "lx_thread2"];
const AIR_DEFAULTS = {
    freq: 0,
    trith: 1,
    absence_timeout: 30,
...
const toBuffer = (data) => {
    ...
    if (Array.isArray(data))
        return Buffer.from(data);
    if (data && typeof data === "object" && Buffer.isBuffer(data.data)) {
        return data.data;
    }
    return null;
};
const actionFromIdOrStr = (actionId, actionStr) => {
    ...
    if (typeof str === "string" && ACTIONS.includes(str)) {
        return str;
    }
    return undefined;
};
const decodeButtonAction = (msg) => {
    if (msg.data && typeof msg.data === "object" && !Buffer.isBuffer(msg.data) && msg.data.type !== "Buffer") {
        const data = msg.data;
...
const decodeAirPayloadFields = (buf, offset) => {
    // layout from offset:
...
const decodeAirConfigMessage = (msg) => {
    if (msg.data && typeof msg.data === "object" && !Buffer.isBuffer(msg.data) && msg.data.type !== "Buffer") {
        const d = msg.data;
        if (d.freq !== undefined ||
            d.trith !== undefined ||
            ...
            if (d.status !== undefined)
                out.config_status = d.status;
            if (d.format_version !== undefined || d.protocol_version !== undefined) {
                out.protocol_version = d.format_version ?? d.protocol_version;
            }
            if (d.freq !== undefined)
                out.freq = d.freq;
            if (d.trith !== undefined)
                out.trith = d.trith;
            if (d.absence_timeout !== undefined)
                out.absence_timeout = d.absence_timeout;
            if (d.radar_enable !== undefined)
                out.radar_enable = d.radar_enable === true || d.radar_enable === 1;
            if (d.lx_interval !== undefined)
                out.lx_interval = d.lx_interval;
            if (d.sht_interval !== undefined)
                out.sht_interval = d.sht_interval;
            if (d.lx_thread1 !== undefined)
                out.lx_thread1 = d.lx_thread1;
            if (d.lx_thread2 !== undefined)
                out.lx_thread2 = d.lx_thread2;
            return out;
        }
    }
    const buf = toBuffer(msg.data);
...
const airFromAttrs = (msg) => {
    const d = msg.data || {};
    const out = {};
    if (d.protocolVersion !== undefined || d["0"] !== undefined || d[0] !== undefined) {
        out.protocol_version = d.protocolVersion ?? d["0"] ?? d[0];
    }
    if (d.freq !== undefined || d["1"] !== undefined || d[1] !== undefined)
        out.freq = d.freq ?? d["1"] ?? d[1];
    if (d.trith !== undefined || d["2"] !== undefined || d[2] !== undefined)
        out.trith = d.trith ?? d["2"] ?? d[2];
...
const currentAirState = (meta) => {
    const s = meta.state || {};
    const out = { ...AIR_DEFAULTS };
...
const buildSetPayload = (state, mask) => ({
    format_version: 0x01,
    mask,
...
const validateLocal = (key, value, state) => {
    switch (key) {
        case "freq":
            if (Number(value) < 0 || Number(value) > 4)
                throw new Error("freq must be 0..4");
            break;
        case "trith":
            if (Number(value) < 1 || Number(value) > 10)
                throw new Error("trith must be 1..10");
            break;
        case "absence_timeout":
            if (Number(value) < 0 || Number(value) > 510)
                throw new Error("absence_timeout must be 0..510");
            break;
        case "radar_enable":
            break;
        case "lx_interval":
            if (Number(value) < 2 || Number(value) > 60000)
                throw new Error("lx_interval must be 2..60000");
            break;
        case "sht_interval":
            if (Number(value) < 10 || Number(value) > 60000)
                throw new Error("sht_interval must be 10..60000");
            break;
        case "lx_thread1":
        case "lx_thread2": {
            ...
            if (t2 <= t1)
                throw new Error("lx_thread2 must be > lx_thread1");
            break;
        }
    }
};
...
const tzLocal = {
    air_config: {
        ...
        convertSet: async (entity, key, value, meta) => {
            ...
            try {
                await entity.command("linknlinkAirConfig", "setConfig", payload, {
                    ...MFC,
        ...
        convertGet: async (entity) => {
            await entity.command("linknlinkAirConfig", "getConfig", {}, MFC);
        },
    },
};
const airExposes = [
...
exports.definitions = [
    {
        ...
        extend: [
            m.deviceAddCustomCluster("linknlinkButtonAction", {
                ...
                commands: {
                    buttonAction: {
                        ...
                        name: "buttonAction",
                        parameters: [
                            { name: "action_id", type: zigbee_herdsman_1.Zcl.DataType.UINT8 },
                            { name: "action_str", type: zigbee_herdsman_1.Zcl.DataType.CHAR_STR },
                        ],
                    },
                },
                commandsResponse: {},
            ...
            m.deviceAddCustomCluster("linknlinkAirConfig", {
                ...
                manufacturerCode: MANUFACTURER_CODE,
                attributes: {
                    protocolVersion: { ID: 0x0000, name: "protocolVersion", type: zigbee_herdsman_1.Zcl.DataType.UINT8 },
                    freq: { ID: 0x0001, name: "freq", type: zigbee_herdsman_1.Zcl.DataType.UINT8 },
                    trith: { ID: 0x0002, name: "trith", type: zigbee_herdsman_1.Zcl.DataType.UINT8 },
                    absenceTimeout: { ID: 0x0003, name: "absenceTimeout", type: zigbee_herdsman_1.Zcl.DataType.UINT16 },
                    radarEnable: { ID: 0x0004, name: "radarEnable", type: zigbee_herdsman_1.Zcl.DataType.BOOLEAN },
                    lxInterval: { ID: 0x0005, name: "lxInterval", type: zigbee_herdsman_1.Zcl.DataType.UINT16 },
                    shtInterval: { ID: 0x0006, name: "shtInterval", type: zigbee_herdsman_1.Zcl.DataType.UINT16 },
                    lxThread1: { ID: 0x0007, name: "lxThread1", type: zigbee_herdsman_1.Zcl.DataType.UINT16 },
                    lxThread2: { ID: 0x0008, name: "lxThread2", type: zigbee_herdsman_1.Zcl.DataType.UINT16 },
                },
                commands: {
                    setConfig: {
                        ID: 0x01,
                        name: "setConfig",
                        parameters: [
                            { name: "format_version", type: zigbee_herdsman_1.Zcl.DataType.UINT8 },
                            { name: "mask", type: zigbee_herdsman_1.Zcl.DataType.UINT16 },
                            { name: "freq", type: zigbee_herdsman_1.Zcl.DataType.UINT8 },
                            { name: "trith", type: zigbee_herdsman_1.Zcl.DataType.UINT8 },
                            { name: "absence_timeout", type: zigbee_herdsman_1.Zcl.DataType.UINT16 },
                            { name: "radar_enable", type: zigbee_herdsman_1.Zcl.DataType.UINT8 },
                            { name: "lx_interval", type: zigbee_herdsman_1.Zcl.DataType.UINT16 },
                            { name: "sht_interval", type: zigbee_herdsman_1.Zcl.DataType.UINT16 },
                            { name: "lx_thread1", type: zigbee_herdsman_1.Zcl.DataType.UINT16 },
                            { name: "lx_thread2", type: zigbee_herdsman_1.Zcl.DataType.UINT16 },
                        ],
                    },
                    getConfig: {
                        ID: 0x02,
                ...
                commandsResponse: {
                    configStatus: {
                        ...
                        name: "configStatus",
                        parameters: [
                            { name: "status", type: zigbee_herdsman_1.Zcl.DataType.UINT8 },
                            { name: "format_version", type: zigbee_herdsman_1.Zcl.DataType.UINT8 },
                            { name: "mask", type: zigbee_herdsman_1.Zcl.DataType.UINT16 },
                            { name: "freq", type: zigbee_herdsman_1.Zcl.DataType.UINT8 },
                            { name: "trith", type: zigbee_herdsman_1.Zcl.DataType.UINT8 },
        ...
        configure: async (device) => {
            ...
            try {
                await ep.command("linknlinkAirConfig", "getConfig", {}, MFC);
            }
            catch {
                try {
                    await ep.read("linknlinkAirConfig", ["freq", "trith", "absenceTimeout", "radarEnable", "lxInterval", "shtInterval", "lxThread1", "lxThread2"], MFC);
                }
                catch {
                    // ignore
                }
        ...
        onEvent: async (event) => {
            if (event.type !== "deviceAnnounce" && event.type !== "start")
                return;
