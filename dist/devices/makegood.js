...
const countdownConverter = {
    from: (value) => (value == null || value === false ? 0 : Number(value)),
...
const energyConverter = {
    from: (value, meta) => {
        ...
        if (value == null)
            return previous;
        const increment = tuya.valueConverter.divideBy1000.from(value);
        // Only add positive deltas; ignore zero reports and any reset/rollover.
        if (increment > 0)
            return Number((previous + increment).toFixed(3));
        return previous;
    },
    to: (value) => value,
};
...
const clampBacklight = (value, min, max) => {
    const numeric = Number(value);
    if (!Number.isFinite(numeric))
        return min;
    return Math.max(min, Math.min(max, Math.round(numeric)));
};
const backlightColorConverter = (channelCount) => {
    const length = 1 + channelCount * BACKLIGHT_CHANNEL_SIZE;
    const offsetOf = (index) => 1 + index * BACKLIGHT_CHANNEL_SIZE;
    ...
    return {
        from(value) {
            ...
            const result = { mode: buffer[0] === 1 ? "multi" : "single" };
...
const channelBacklightFeature = (property, label) => e
    ...
    .withFeature(e.numeric("off_hue", ea.STATE_SET).withValueMin(0).withValueMax(359).withValueStep(1).withDescription("Hue while off — multi mode only"))
    .withFeature(e
...
const backlightColorExpose = (labels) => {
    let composite = e
