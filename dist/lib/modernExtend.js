...
exports.thermostatUi = thermostatUi;
...
function identify(args = {}) {
    ...
    const exposes = (0, utils_1.exposeEndpoints)(identifyExpose, endpointNames);
    ...
    const toZigbee = [endpointNames ? { ...baseConverter, endpoints: endpointNames } : baseConverter];
...
function fanControl(args = {}) {
    const { cluster = "hvacFanCtrl", endpointNames = undefined, reporting = { min: 0, max: "1_HOUR", change: 0 }, bind = true } = args;
    const fanExpose = exposes_1.presets.fan().withState("state").withModes(Object.keys(constants.fanMode));
    const exposes = (0, utils_1.exposeEndpoints)(fanExpose, endpointNames);
    const fromZigbee = [fz.fan];
    const toZigbee = [tz.fan_mode];
    const result = { exposes, fromZigbee, toZigbee, isModernExtend: true };
    const configure = [];
    if (bind) {
    ...
    if (reporting) {
        configure.push(setupConfigureForReporting(cluster, "fanMode", {
            config: reporting,
            access: exposes_1.access.STATE_GET,
...
function thermostatUi(args = {}) {
    const { cluster = "hvacUserInterfaceCfg", endpointNames = undefined, reporting = { min: 10, max: "1_HOUR", change: 0 }, bind = true } = args;
    const exposes = [
        ...(0, utils_1.exposeEndpoints)(exposes_1.presets.enum("temperature_display_mode", exposes_1.access.ALL, Object.values(constants.temperatureDisplayMode)).withDescription("Temperature display mode"), endpointNames),
        ...(0, utils_1.exposeEndpoints)(exposes_1.presets.enum("keypad_lockout", exposes_1.access.ALL, Object.values(constants.keypadLockoutMode)).withDescription("Keypad lockout mode"), endpointNames),
    ];
    const fromZigbee = [fz.hvac_user_interface];
    const toZigbee = [tz.thermostat_temperature_display_mode, tz.thermostat_keypad_lockout];
    const result = { exposes, fromZigbee, toZigbee, isModernExtend: true };
    const configure = [];
    if (bind) {
        configure.push(setupConfigureForBinding(cluster, "input", endpointNames));
    ...
    if (reporting) {
        configure.push(setupConfigureForReporting(cluster, "keypadLockout", {
            config: reporting,
            access: exposes_1.access.STATE_GET,
            endpointNames: endpointNames,
        }));
...
function occupancy(args = {}) {
    ...
    const exposes = (0, utils_1.exposeEndpoints)(exposes_1.presets.occupancy().withAccess(exposes_1.access.STATE_GET), endpointNames);
    const fromZigbee = [
        {
            ...
            convert: (model, msg, publish, options, meta) => {
                if ("occupancy" in msg.data && (!endpointNames || endpointNames.includes((0, utils_1.getEndpointName)(msg, model, meta).toString()))) {
                    const propertyName = endpointNames ? (0, utils_1.postfixWithEndpointName)("occupancy", msg, model, meta) : "occupancy";
...
function iasWarning(args = {}) {
    ...
    if (maxDuration) {
        exposes.push(exposes_1.presets
            .numeric("max_duration", exposes_1.access.ALL)
            .withUnit("s")
...
function numeric(args) {
    const { name, cluster, attribute, description, zigbeeCommandOptions, unit, reporting, valueMin, valueMax, valueStep, scale, label, entityCategory, precision, fzConvert, homeassistant, } = args;
    ...
    if (nameArray.length > 1) {
        (0, node_assert_1.default)(endpoints && endpoints.length === nameArray.length, "If multiple names are provided, endpointNames must be provided and have the same length.");
    }
    if (labelArray.length > 1) {
        (0, node_assert_1.default)(endpoints && endpoints.length === labelArray.length, "If multiple labels are provided, endpointNames must be provided and have the same length.");
    }
    if (descriptionArray.length > 1) {
        (0, node_assert_1.default)(endpoints && endpoints.length === descriptionArray.length, "If multiple descriptions are provided, endpointNames must be provided and have the same length.");
    }
    if (homeassistantArray.length > 1) {
        (0, node_assert_1.default)(endpoints && endpoints.length === homeassistantArray.length, "If multiple homeassistants are provided, endpointNames must be provided and have the same length.");
    }
    if (unitArray.length > 1) {
        (0, node_assert_1.default)(endpoints && endpoints.length === unitArray.length, "If multiple units are provided, endpointNames must be provided and have the same length.");
    }
    if (valueMinArray.length > 1) {
        (0, node_assert_1.default)(endpoints && endpoints.length === valueMinArray.length, "If multiple valueMins are provided, endpointNames must be provided and have the same length.");
    }
    if (valueMaxArray.length > 1) {
        (0, node_assert_1.default)(endpoints && endpoints.length === valueMaxArray.length, "If multiple valueMaxs are provided, endpointNames must be provided and have the same length.");
    }
    if (valueStepArray.length > 1) {
        (0, node_assert_1.default)(endpoints && endpoints.length === valueStepArray.length, "If multiple valueSteps are provided, endpointNames must be provided and have the same length.");
    }
    if (precisionArray.length > 1) {
        (0, node_assert_1.default)(endpoints && endpoints.length === precisionArray.length, "If multiple precisions are provided, endpointNames must be provided and have the same length.");
    }
    const exposes = [];
    const createExpose = (name, description, endpoint, unit, valueMin, valueMax, valueStep, label, homeassistant) => {
        ...
        if (entityCategory)
            expose = expose.withCategory(entityCategory);
        if (homeassistant)
            expose = expose.withHomeAssistant(homeassistant);
        return expose;
    };
    // Generate for multiple endpoints only if required.
    if (!endpoints) {
        exposes.push(createExpose(nameArray[0], descriptionArray[0], undefined, unitArray[0], valueMinArray[0], valueMaxArray[0], valueStepArray[0], labelArray[0], homeassistantArray[0]));
    }
    else {
        for (const [i, endpoint] of endpoints.entries()) {
            exposes.push(createExpose(nameArray[i] ?? nameArray[0], descriptionArray[i] ?? descriptionArray[0], endpoint, unitArray[i] ?? unitArray[0], valueMinArray[i] ?? valueMinArray[0], valueMaxArray[i] ?? valueMaxArray[0], valueStepArray[i] ?? valueStepArray[0], labelArray[i] ?? labelArray[0], homeassistantArray[i] ?? homeassistantArray[0]));
        }
    }
    const fromZigbee = [
        {
            ...
            convert: fzConvert ??
                ((model, msg, publish, options, meta) => {
                    ...
                    if (attributeValue !== undefined) {
                        const endpointName = endpoints ? (0, utils_1.getEndpointName)(msg, model, meta) : undefined;
...
function binary(args) {
    const { name, valueOn, valueOff, cluster, attribute, description, zigbeeCommandOptions, endpointName, reporting, label, entityCategory, homeassistant, } = args;
    const descriptionArray = Array.isArray(description) ? description : [description];
    const labelArray = Array.isArray(label) ? label : [label];
    const homeassistantArray = Array.isArray(homeassistant) ? homeassistant : [homeassistant];
    let endpoints;
    ...
    const exposes = [];
    const createExpose = (description, endpoint, label, homeassistant) => {
    ...
    if (!endpoints) {
        ...
        for (const [i, endpoint] of endpoints.entries()) {
            exposes.push(createExpose(descriptionArray[i] ?? descriptionArray[0], endpoint, labelArray[i] ?? labelArray[0], homeassistantArray[i] ?? homeassistantArray[0]));
        }
    }
    const fromZigbee = [
        {
...
function thermostat(args) {
    const { localTemperature = undefined, localTemperatureCalibration = undefined, setpoints, setpointsLimit = {}, systemMode = undefined, runningState = undefined, runningMode = undefined, fanMode = undefined, acLouverPosition = undefined, piHeatingDemand = undefined, temperatureSetpointHold = false, temperatureSetpointHoldDuration = false, endpoint = undefined, ctrlSeqeOfOper = undefined, programmingOperationMode = undefined, setpointChangeSource = undefined, weeklySchedule = undefined, } = args;
    ...
    if (acLouverPosition) {
        ...
        if (!acLouverPosition.configure?.skip) {
            configure.push(setupConfigureForReporting("hvacThermostat", "acLouverPosition", {
                config: acLouverPosition.configure?.reporting ?? repConfigChange0,
                access: acLouverPosition.configure?.access ?? exposes_1.access.STATE_GET,
                endpointNames: endpointNames,
            }));
        }
    }
