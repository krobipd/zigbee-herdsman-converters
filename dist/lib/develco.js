...
exports.develcoModernExtend = {
    ...
    faultStatus: () => {
        const reliabilityExpose = exposes_1.presets
            .enum("reliability", exposes_1.access.STATE, ["no_fault_detected", "unreliable_other", "process_error"])
            .withDescription("Indicates reason if any fault");
        const faultExpose = exposes_1.presets.binary("fault", exposes_1.access.STATE, true, false).withDescription("Indicates whether the device are in fault state");
        const fromZigbee = [
            {
                ...
                convert: (_model, msg, _publish, _options, _meta) => {
                    ...
                    if (msg.data.reliability !== undefined) {
                        ...
                        result.reliability = (0, utils_1.getFromLookup)(msg.data.reliability, lookup);
                    }
            ...
            async (device, _coordinatorEndpoint, _definition) => {
                for (const ep of device.endpoints) {
                    if (ep.supportsInputCluster("genBinaryInput")) {
                        await (0, modernExtend_1.setupAttributes)(ep, _coordinatorEndpoint, "genBinaryInput", [
